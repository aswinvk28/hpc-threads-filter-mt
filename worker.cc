...
void execute_section_wise_frames(FrameParams * frameParams, float * data, vector<float>& pointers);
  ...
  #pragma omp parallel
  {
    #pragma omp master
    {
      for(ptrdiff_t i = st; i < st+frameParams->num_subtasks; i++) {
        #pragma omp task priority(st+frameParams->num_subtasks-i)
        {
  ...
  #pragma omp parallel
  {
    #pragma omp master
    {
      for(ptrdiff_t i = 0; i < frameParams->num_tasks; i++) {
        #pragma omp task priority((frameParams->num_frames-frame_no)*(frameParams->num_tasks)+frameParams->num_tasks-i)
        {
